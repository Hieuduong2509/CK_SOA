...
def list_projects(
    ...
    if client_id:
        # Client can see all their projects (including drafts)
        projects = get_projects_by_client(db, client_id)
    elif freelancer_id:
        # Freelancers can only see OPEN projects (approved by admin)
        projects = get_projects_by_freelancer(db, freelancer_id)
        projects = [p for p in projects if p.status == ProjectStatus.OPEN]
    else:
        # Public listing - only show OPEN projects (approved by admin)
