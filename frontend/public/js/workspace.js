    ...
    try {
        ...
        const freelancerId = project.accepted_bid_id ? 
            (await fetch(`${API_BASE}/api/v1/projects/${projectId}/bids`, {
                ...
                return acceptedBid?.freelancer_id;
            })) : null;
...
async function loadMilestones(projectId) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            const isClient = user && (user.role === 'client' || user.role?.value === 'client' || user.role?.name === 'client');
            const isFreelancer = user && (user.role === 'freelancer' || user.role?.value === 'freelancer' || user.role?.name === 'freelancer');
...
async function loadProjectFiles(projectId) {
    ...
    try {
        ...
        // Convert string attachments to object format for consistency
        ...
        if (attachments.length === 0) {
            filesList.innerHTML = `
                <div style="text-align: center; padding: 2rem;">
                    <p style="color: var(--text-secondary); margin-bottom: 1rem;">Chưa có tệp đính kèm.</p>
                    ${isOwner ? `
                        <label for="fileUpload" class="btn btn-primary" style="cursor: pointer; display: inline-block; margin-bottom: 0.5rem;">
                        ...
                        <p style="font-size: 0.875rem; color: var(--text-secondary); margin-top: 0.5rem;">Giới hạn: 50MB mỗi file</p>
                    ` : ''}
                </div>
            `;
            return;
        ...
        filesList.innerHTML = `
            <div style="margin-bottom: 1rem;">
                ${isOwner ? `
                    <label for="fileUpload" class="btn btn-primary btn-small" style="cursor: pointer; display: inline-block; margin-bottom: 0.5rem;">
                        <i class="fas fa-upload"></i> Tải tệp lên
                    </label>
                    <input type="file" id="fileUpload" multiple style="display: none;" onchange="handleFileUpload(${projectId}, this.files)">
                    <p style="font-size: 0.875rem; color: var(--text-secondary); margin-top: 0.25rem;">Giới hạn: 50MB mỗi file</p>
                ` : ''}
            </div>
            <div style="display: flex; flex-direction: column; gap: 0.75rem;">
                ${attachments.map((attachment, index) => {
                    ...
                    return `
                        <div class="file-item" style="display: flex; justify-content: space-between; align-items: center; padding: 1rem; background: var(--bg-gray); border-radius: var(--radius-lg); border: 1px solid var(--border-color);">
                            <div style="flex: 1; display: flex; align-items: center; gap: 1rem;">
                                <div style="width: 40px; height: 40px; background: var(--primary-color); border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; color: white; font-size: 1.25rem;">
                                    <i class="fas fa-file"></i>
                                </div>
                                <div style="flex: 1;">
                                    <a href="#" onclick="downloadProjectFile(${projectId}, ${index}, event)" style="color: var(--primary-color); text-decoration: none; font-weight: 500; display: block; margin-bottom: 0.25rem; cursor: pointer;">
...
async function deleteProjectFile(projectId, attachmentIndex, filename) {
    ...
    try {
        ...
        if (response.ok) {
            ...
            console.log('Deleted project attachments:', result.project?.attachments);
            
            // Reload project first to get updated data
            try {
                currentProject = await getProject(projectId);
                console.log('Project reloaded after delete, attachments:', currentProject?.attachments);
            } catch (error) {
